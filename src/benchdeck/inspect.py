...
def inspect_run(run_dir: Path) -> dict[str, Any]:
    ...
    judged = int(metadata.get("judged_cases") or tally.get("cases_judged") or tally.get("cases_completed") or 0)
    if judged < planned:
    ...
    for agent, results in snapshot.results.items():
        for result in results:
            ...
            if result.get("judge_transcript") == result.get("final_output") and result.get("final_output"):
                warnings.append(
                    f"{agent} case {result.get('case_id')} stores candidate output as judge_transcript."
                )
